    ...
    sentences.forEach((sentWords, sIdx) => {
        ...
        for (let i = 0; i < sentWords.length; i++) {
            ...
            if (i === sentWords.length - 1) {
                ...
                forceSplit = true;
            } else if (curChunkWords.length >= 12 && isWeakPunct && remainingWords >= 3) {
                forceSplit = true; 
            }
...
export default function App() {
  // 强制清除旧缓存体系，应用最新的默认无脑免费配置 (智谱 GLM-4-Flash)
  ...
  const startProcessing = async () => {
    ...
    try {
      ...
      for (let i = 0; i < parsedSentences.length; i += chunkSize) {
        ...
        4. CRITICAL: You MUST translate EVERY SINGLE sentence provided. Return EXACTLY ${chunk.length} items mapping exactly to the input "id". Ensure contextual flow.
        ...
        while (!chunkSuccess && retryCount < maxRetries) {
          try {
            ...
            chunk.forEach(sent => {
                sent.zh = transDict[sent.id] || "（防漏译：大模型未生成此句中文，请手动补全）";
            });
  ...
  const startVideoExport = async () => {
      ...
      try {
          ...
          const drawFrame = () => {
              ...
              // Area A: 16:9 Image (Top-aligned under header)
              ...
              // Area B: Grid Stacking Subtitles
              if (activeSentence) {
        ...
        <div className="flex-1 overflow-y-auto bg-gray-50 flex flex-col">
          <div className="p-8 max-w-3xl mx-auto w-full space-y-6 flex-1">
            <div className="border-b border-gray-200 pb-4">
              ...
              <p className="text-sm text-gray-500 mt-2">完全剥离苹果缓存干预的高配直驱版。</p>
            </div>
            ...
            <div className="bg-white p-5 rounded-xl border shadow-sm space-y-4">
              ...
              <div>
                <label className="text-sm font-bold block mb-2">2. 粘贴原文字幕结构段</label>
                <textarea className="w-full h-24 p-3 text-xs border border-gray-300 rounded-lg resize-none outline-none focus:ring-1 focus:ring-blue-500 bg-gray-50" value={formData.rawText} onChange={e => setFormData({...formData, rawText: e.target.value})} placeholder="粘贴原文供大模型划分新闻1、新闻2及提取日期..."></textarea>
              </div>
            </div>
                          ...
                          <div key={sent.id}>
                            <div className={`rounded-lg border transition-all duration-200 ${isSentenceActive ? 'border-sky-400 bg-sky-50/20 shadow-md ring-1 ring-sky-200' : 'border-gray-200 bg-white hover:border-gray-300'}`}>
                              ...
                              <div className="p-2 space-y-1.5">
                                <div className="text-[9px] font-bold text-gray-400">英文 20 词强制切分轴</div>
                                {sent.chunks.map((chunk, cIdx) => {
                                    const isChunkActive = currentTime >= chunk.start && currentTime <= chunk.end;
                                    return (
                                        <div key={chunk.id} className={`flex items-start space-x-2 rounded p-1.5 border ${isChunkActive ? 'border-blue-400 bg-blue-50' : 'border-gray-100 bg-gray-50'}`}>
                                            ...
                                            <textarea value={chunk.en} onChange={(e) => { const n=[...sentences]; n[sentIdx].chunks[cIdx].en=e.target.value; n[sentIdx].en = n[sentIdx].chunks.map(c=>c.en).join(" "); setSentences(n); }} className="flex-1 text-[11px] font-medium text-gray-700 bg-transparent outline-none resize-none h-[30px]" />
                                        </div>
                                    )
                                })}
                              </div>
  ...
  return (
    <div className="flex h-screen w-screen bg-gray-900 text-gray-800 font-sans overflow-hidden">
      {/* 隐形真理锚点：唯一被允许发声的宿主，杜绝在 React Lifecycle 里被销毁重置 */}
      <audio 
        ref={audioRef} 
